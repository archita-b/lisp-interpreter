...
const sExprEval = input => {
	...
	if (!splForms.includes(symbol)) {
		...
		if (typeof func !== 'function')
			return [symbol = func, input.slice(1)];
...
const exprParser = input => {
	...
	return booleanParser(input) || numParser(input) || stringParser(input)
		|| sExprEval(input) || symbolEval(input);
}
...
const quoteParser = input => {
	...
	// const parsed = sExprParser(input) || symbolParser(input);
	const parsed = symbolParser(input) || sExprParser(input);
